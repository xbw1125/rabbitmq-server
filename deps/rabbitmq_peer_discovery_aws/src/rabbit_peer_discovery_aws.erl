...
list_nodes() ->
    ...
    _ = rabbit_log:debug("Started rabbitmq_aws"),
...
api_get_request(Service, Path) ->
    case rabbitmq_aws:get(Service, Path) of
        {ok, {_Headers, Payload}} ->
            _ = rabbit_log:debug("AWS request: ~s~nResponse: ~p~n",
                             [Path, Payload]),
            {ok, Payload};
...
instance_id() ->
    case httpc:request(get, {?INSTANCE_ID_URL, []},
        ...
        {ok, {{_, 200, _}, _, Value}} ->
            _ = rabbit_log:debug("Fetched EC2 instance ID from ~p: ~p",
                             [?INSTANCE_ID_URL, Value]),
            Value;
        Other ->
            _ = rabbit_log:error("Failed to fetch EC2 instance ID from ~p: ~p",
