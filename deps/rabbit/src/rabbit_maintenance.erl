...
drain() ->
    case is_enabled() of
        true  -> do_drain();
        false -> _ = rabbit_log:warning("Feature flag `~s` is not enabled, draining is a no-op", [?FEATURE_FLAG])
    end.
...
revive() ->
    case is_enabled() of
        true  -> do_revive();
        false -> _ = rabbit_log:warning("Feature flag `~s` is not enabled, reviving is a no-op", [?FEATURE_FLAG])
    end.
